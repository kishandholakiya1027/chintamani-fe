...
const Sidebar = ({setFilter, filter, isJewellery}: any) => {
  ...
  const changeFilter = (name: string, value: string) => {
    if (filter?.[name]?.includes(value)) {
      ...
      if (data?.length) {
        setFilter({
          ...filter,
          [name]: filter?.[name]?.filter((item: string) => item !== value),
        });
      } else {
        delete filter[name];
        setFilter({...filter});
      }
    } else setFilter({...filter, [name]: [...(filter?.[name] || []), value]});
  };
            ...
            <div>
              ...
              <div className="pr-4">
                <MultiRangeSlider
                  ...
                  maxValue={filter?.maxPrice || 13060}
                  labels={[filter?.minPrice?.toString() || "0", filter?.maxPrice?.toString() || "13060"]}
                  ruler={false}
            ...
            <div>
              ...
              <div className="pr-4">
                <MultiRangeSlider
                  ...
                  step={5}
                  subSteps={true}
                  minValue={filter?.mincarat || 0}
                  maxValue={filter?.maxcarat || 100}
                  labels={[filter?.mincarat?.toString() || "0", filter?.maxcarat?.toString() || "100"]}
                  ruler={false}
                  onInput={() => {
                ...
                {clarities?.map((clarity: diamondProperty) => {
                  ...
                  return (
                    <Button
                      ...
                      variant={"outline"}
                      className={`border-[#211c50]  ${
                        filter?.Clarity &&
                        filter?.Clarity?.includes(clarity?.name)
                          ? "bg-[#211c50] text-[#fff]"
                          : "text-[#211c50]"
                      }  hover:bg-[#211c50] hover:text-[#fff]  w-full py-[8px] px-[2px] break-words break-all whitespace-normal`}
                    >
                      {extractedString}
                    </Button>
                  );
                ...
                {cuts?.map((cut: diamondProperty) => {
                  ...
                  return (
                    <Button
                      ...
                      variant={"outline"}
                      className={`border-[#211c50] hover:bg-[#211c50] hover:text-[#fff] ${
                        filter?.Cut && filter?.Cut?.includes(cut?.name)
                          ? "bg-[#211c50] text-[#fff]"
                          : "text-[#211c50]"
                      }  w-full py-[8px] px-[2px] break-words break-all whitespace-normal`}
                    >
                      {extractedString}
                    </Button>
                  );
                ...
                {colors?.map((color: diamondProperty) => {
                  ...
                  return (
                    <Button
                      ...
                      variant={"outline"}
                      className={`border-[#211c50] hover:bg-[#211c50] ${
                        filter?.Color && filter?.Color?.includes(color?.name)
                          ? "bg-[#211c50] text-[#fff]"
                          : "text-[#211c50]"
                      } hover:text-[#fff]  w-full py-[8px] px-[2px] break-words break-all whitespace-normal`}
                    >
                      {extractedString}
                    </Button>
                  );
