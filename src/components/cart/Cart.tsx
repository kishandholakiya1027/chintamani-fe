...
const Index = () => {
  ...
  const removeFromCart = async () => {
    if (user?.id) {
      ...
      dispatch(deleteItem({ productId: openPopup }));
      setOpenPopup("");
    }
  };
          ...
          <div className="mx-auto max-w-5xl justify-center px-6 h-[calc(100vh-64px)] md:space-x-6 xl:px-0 overflow-y-auto">
            <div className="px-6 py-4">
              <div className="rounded-lg md:w-full">
                ...
                {cartProduct?.length ? (
                  (cartProduct || [])?.map((products: any, index: number) => {
                    let product = products?.product || products;
                    let qty = products?.quantity;
                    return (
                      <div
                        className="justify-between mb-6 rounded-lg bg-white p-4 shadow-[0px_.125rem_.25rem_0px_rgba(0,0,0,0.075)] overflow-hidden sm:flex sm:justify-start"
                        key={index}
                      >
                        <img
